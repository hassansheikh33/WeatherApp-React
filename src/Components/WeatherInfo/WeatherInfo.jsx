...
export default function WeatherInfo() {
  ...
  const date = `${days[dateObj.getDay()]}, ${
  ...
  function dataFunc(data) {
    setForecasted(true);
    let forecastArr = [];
    if (selectRef.current.value >= 3 && data.forecast.forecastday.length <= 3) {
      weatherCxt.setExpired(true);
    } else {
      weatherCxt.setExpired(false);
    }
  ...
  const forecastedData = (
    <div className={classes.daysContainer}>
      {weatherCxt.forecast.map((day) => {
        return (
          <Card className={classes.forecastDay} key={day.id}>
            <h5>{day.date}</h5>
  ...
  function forecastHandler(e) {
    ...
    getWeather(
      "forecast",
      dataFunc,
      `&q=${weatherCxt.city}&days=${numDays}&aqi=no&alerts=no`
    );
  }
  return (
    <>
      <section className={classes.currentSection}>
        <h2 style={{ marginBottom: "0" }}>
          City: {weatherCxt.city}, {weatherCxt.country}
        </h2>
        <h3>{date}</h3>
        <img
        ...
        <h2
          className={
            weatherCxt.weather === "Sunny"
              ? classes.sunny
              : weatherCxt.weather.includes("rain")
              ? classes.rainy
              : classes.cloudy
          }
          style={{ marginBottom: "0" }}
        >
          {weatherCxt.weather} , {weatherCxt.temp}°C
        </h2>
        ...
        <form onSubmit={forecastHandler}>
          ...
          <span> Days </span>
          <button type="submit" className={classes.forecastBtn}>
            Check
          </button>
        </form>
      </section>
      ...
      {!isLoading && error && (
        <h2>Error occured while cheking Forecast: {error.message}</h2>
      )}
      {!isLoading && !error && forecasted && (
        <section className={classes.sectionFor}>
          <h2>
            {selectRef.current.value === "1"
              ? `Tomorrow's Weather:`
              : `Upcoming ${selectRef.current.value} Days Weather:`}{" "}
          </h2>
          {forecastedData}
          {weatherCxt.expired && (
            <p style={{ color: "red" }}>
              API key has expired! Cannot fetch weather of more than 2 days. You
              can signup{" "}
              <a
              ...
              and put your own API key in useFetch hook
            </p>
          )}
        </section>
      )}
